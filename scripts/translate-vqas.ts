...
async function main() {
  ...
  const untranslatedVQAs = vqas.filter(v => !v.question.de)
  ...
  for (const vqaGroup of Object.values(groups)) {
    const texts = vqaGroup.map(vqa => {
      return vqa.question.en + "\n" + vqa.answer.en
    })
    ...
    amountTranslated += vqaGroup.length
