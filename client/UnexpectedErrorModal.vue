<template>
  <b-modal id="unexpectedErrorModal" v-model="showErrorModal" ok-only size="lg">
    <template slot="modal-header" slot-scope="{ close }">
      <div v-if="serverResponse">
        ...
        <pre
          >{{ serverResponse.status }} {{ serverResponse.statusText }} from {{
            error.config.url
          }}</pre
        >
        <p>
    ...
    <template slot="modal-ok"> Continue </template>
  </b-modal>
</template>
...
export default class UnexpectedErrorModal extends Vue {
  ...
  get serverResponse(): AxiosResponse | null {
    return "response" in this.error && this.error.response
      ? this.error.response
      : null
  }
  ...
  get serverResponseHTML(): string | null {
    if (!this.serverResponse) return null

    if (_.isString(this.serverResponse.data)) return this.serverResponse.data
    else
