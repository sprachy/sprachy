...
const learnyStore = useLearnyStore()

const savedQuestionId = localStorage.getItem("mlLastQuestionId")

let initialQuestionIndex = props.allTasks.findIndex(
...
const state = defineState({
  ...
  get task() {
    return new VQATask(props.allTasks[this.questionIndex])
  },
  ...
  get nextTask() {
    const def = props.allTasks[this.questionIndex + 1]
    return def ? new VQATask(def) : null
  },
})

async function toNextTask() {
  const newLemmas = uniq(state.task.questionLemmas).filter(w => !learnyStore.knowsLemma(w))
  if (newLemmas.length) {
...
async function toggleEditMode() {
  if (state.editingVQA) {
    props.allTasks[state.questionIndex] = state.editingVQA
    ...
    <div v-if="!state.editingVQA" class="task">
      ...
      <div :key="state.task.def.id">
        <p class="question hover-translate" :data-tooltip="state.task.questionEn">
          <span
            :class="{ token: true, punctuation: token.value.match(/^[.,!?]$/), new: !learnyStore.knowsLemma(token.value) }"
