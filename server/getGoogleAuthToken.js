...
export async function getGoogleAuthToken(user, key, scope) {
  const webcrypto = typeof crypto !== 'undefined' ? crypto : new (await import("@peculiar/webcrypto")).Crypto
  ...
  async function sign(content, signingKey) {
    ...
    const binarySignature = await webcrypto.subtle.sign({ name: "RSASSA-PKCS1-V1_5" }, signer, buf)
